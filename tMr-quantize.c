/**
 * tMr-quantize.c - Terminal MIDI Recorder with Quantization
 *
 * Build: clang -framework AudioToolbox -framework IOKit -framework CoreFoundation tMr-quantize.c -o tMr-quantize
 *
 * Keyboard Layout:
 ...
 *   /         = Save MIDI file
 *   `         = Cycle quantize grid (OFF, 1/4, 1/8, 1/16, 1/32)
 *   ESC       = Quit
 */
...
// Quantize grid values (in ticks)
typedef enum {
    QUANTIZE_OFF = 0,
    QUANTIZE_1_4 = TICKS_PER_BEAT,           // Quarter note (480 ticks)
    QUANTIZE_1_8 = TICKS_PER_BEAT / 2,       // Eighth note (240 ticks)
    QUANTIZE_1_16 = TICKS_PER_BEAT / 4,      // Sixteenth note (120 ticks)
    QUANTIZE_1_32 = TICKS_PER_BEAT / 8       // Thirty-second note (60 ticks)
} QuantizeGrid;

static const char* quantizeNames[] = {
...
static const uint16_t GRAVE_KEYCODE = 0x35;  // Backtick/grave accent for quantize toggle
...
// Global state - Quantization
static int quantizeIndex = 0;  // Index into quantize grid options (0=OFF, 1=1/4, etc.)
static const QuantizeGrid quantizeGrids[] = {
    QUANTIZE_OFF, QUANTIZE_1_4, QUANTIZE_1_8, QUANTIZE_1_16, QUANTIZE_1_32
};
static const int QUANTIZE_OPTIONS = sizeof(quantizeGrids) / sizeof(quantizeGrids[0]);
...
// Quantization function - snaps a tick value to the nearest grid position
static uint32_t quantize_tick(uint32_t tick) {
    QuantizeGrid grid = quantizeGrids[quantizeIndex];
    if (grid == QUANTIZE_OFF) return tick;

    // Round to nearest grid position
    uint32_t gridPos = tick / grid;
    uint32_t remainder = tick % grid;

    if (remainder >= grid / 2) {
        gridPos++;  // Round up
    }

    uint32_t quantizedTick = gridPos * grid;

    // Handle wrap-around at loop boundary
    if (quantizedTick >= totalLoopTicks) {
        quantizedTick = 0;
    }
...
static void note_on(uint8_t note, uint8_t velocity) {
    ...
    if (recording && clockRunning) {
        ...
        if (track->eventCount < MAX_EVENTS_PER_TRACK) {
            ...
            // Apply quantization to the tick
...
static void note_off(uint8_t note) {
    ...
    if (recording && clockRunning) {
        ...
        if (track->eventCount < MAX_EVENTS_PER_TRACK) {
            uint32_t tick = get_current_tick();
            // Apply quantization to note-off as well
            tick = quantize_tick(tick);
...
// Cycle through quantize options
static void cycle_quantize(void) {
    quantizeIndex = (quantizeIndex + 1) % QUANTIZE_OPTIONS;
    update_status_display();
}
...
static void update_status_display(void) {
    ...
    // Quantize indicator
    printf("Q:%s ", quantizeNames[quantizeIndex]);
    ...
    char progName[16];
    strncpy(progName, gmNames[tracks[currentChannel].program], 15);
    progName[15] = '\0';
    printf("P%03d:%.15s ", tracks[currentChannel].program, progName);
...
static void hid_callback(void *context, IOReturn result, void *sender, IOHIDValueRef value) {
    ...
    // GRAVE (backtick) - Cycle quantize
    if (usage == GRAVE_KEYCODE && pressed) {
        cycle_quantize();
        return;
    }
...
int main(void) {
    ...
    printf("tMr-quantize - MIDI Recorder with Quantization\n");
    printf("══════════════════════════════════════════════════\n");
    ...
    printf("`          Cycle quantize (OFF/1/4/1/8/1/16/1/32)\n");
